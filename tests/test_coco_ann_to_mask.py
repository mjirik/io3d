...
def test_coco_ann_to_mask():
    ...
    convert_coco_ann_to_mask.coco_to_mask(
        input_path, output_path, "Vena Cava", output_type=output_type
    )
...
def test_all_masks():
    ...
    output_type = ".png"
    output_path = Path(
        r"H:\biomedical\orig\pilsen_pigs_raw\transplantation\annotations"
    )
    ...
    for fn in filenames:
        for organ_key in organ_keys:
            ...
            # logger.debug(out)
            output_path = io3d.datasets.get_dataset_path(
                "pilsen_pigs", organ_key_file, data_id=int(out[0])
            )
            logger.debug(f"output_path={output_path}, exists={output_path.exists()}")
            convert_coco_ann_to_mask.coco_to_mask(
                fn, output_path, organ_key, output_type=output_type
            )
...
def test_check_mask():
    ...
    organ_key_file = organ_key.lower().replace(" ", "_")
    output_path = io3d.datasets.get_dataset_path(
        "pilsen_pigs", organ_key_file, data_id=data_id
    )
