...
import io3d
import re
from io3d import cachefile as cachef
...
def get_old(dataset_label, file_id, destination_dir=None):
    ...
    print(paths)
    print(file_id)
    pathsf = fnmatch.filter(paths, file_id)
    print(pathsf)
...
def compare_hashes(current_hash: str, expected_hash: str) -> bool:
    ...
    :param current_hash: hash in string form
    :param expected_hash: hash in string form
    :return: True if hashes equals, else False
    """
    if current_hash and expected_hash is None:
        return True
...
def generate_abdominal(size=100, liver_intensity=100, noise_intensity=20, portal_vein_intensity=130,
    ...
    seeds[boundary + 1: boundary + 4, boundary + 1: boundary + 4, 2 * boundary + 1: 2 * boundary + 4] = 1
