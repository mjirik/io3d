...
def test_pilsen_pigs():
    ...
    input_dir = io3d.joinp(
        "pilsen_pigs_raw/transplantation"
    )  # r'H:\biomedical\orig\pilsen_pigs_raw\transplantation'
    input_dir = Path(
        r"G:\.shortcut-targets-by-id\1Vq6QAiJ7cHDJUdgKXjUxx5iw7zk6MMPI\transplantation"
    )
    output_dir = Path(
        r"G:\.shortcut-targets-by-id\1nz7i4Zssar8075yu6J5QpHdph6oi3uF8\pilsen_pigs"
    )
    file_list1 = list(input_dir.glob("*_Ven*"))
    transform_file_structure.prepare_pilsen_pigs_dataset(
        input_dir, output_dir=output_dir, output_format=".mhd"
    )
...
def test_pilsen_pigs_create_in_dataset_dir():
    ...
    input_dir = io3d.joinp(
        "biomedical/orig/pilsen_pigs_raw/transplantation"
    )  # r'H:\biomedical\orig\pilsen_pigs_raw\transplantation'
    output_type = ".mhd"
    output_dir = io3d.datasets.get_dataset_path(
        "pilsen_pigs", "data3d", data_id=1
    ).parents[2]
    # input_dir = r'H:\biomedical\orig\pilsen_pigs_raw\transplantation'
    # output_dir = Path(r"H:\biomedical\orig\pilsen_pigs")
    ...
    transform_file_structure.prepare_pilsen_pigs_dataset(
        ...
        dirname_pattern="{re_out[0]}",
        regexp_pattern="(Tx[0-9]+D_Ven)",
    )
